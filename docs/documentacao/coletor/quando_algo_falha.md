...
> 💡 **O Coletor agora aponta isso antes do envio.** A mesma conferência passou a rodar **na extração**, contra os cadastros que o Coletor sincroniza da PNP. De fábrica ela está em **modo sombra**: não reprova nada, mas registra a divergência nos detalhes do Registro de Extração — então dá para ver o campus errado no dia da extração, em vez de esperar a fila de validação. Ver [Validação referencial]({{ site.baseurl }}/documentacao/usuarios-especializados/contratos/validacao_referencial).
