...
- **Padronização entre instituições.** A Rede Federal tem dezenas de instituições, cada uma com bancos, planilhas e APIs próprios. O contrato define um formato único de entrega — o trabalho de mapear "como sai do meu sistema" para "o que a PNP espera" fica isolado em uma só camada (a **[Configuração de Extração]({{site.baseurl}}/documentacao/coletor/glossario#configuracao-de-extracao)**).
